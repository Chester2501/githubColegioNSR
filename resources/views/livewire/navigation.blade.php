        ...
        <div class="hidden sm:block sm:ml-6">
          <div class="flex space-x-4">
            ...
            @foreach ($categorias as $categoria)
                <a href="{{route('posts.categoria', $categoria)}}" class="text-gray-300 hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium">{{$categoria->name}}</a>
            @endforeach
          ...
          <div x-show="open" x-on:click.away="open=false" class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
            ...
            <a href="{{route('profile.show')}}" class="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1" id="user-menu-item-0">Tu Perfil</a>
             ...
             <form method="POST" action="{{ route('logout') }}">
               @csrf
               <a href="{{ route('logout') }}" class="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1" id="user-menu-item-2"  onclick="event.preventDefault(); this.closest('form').submit();">Cerrar Sesión
 ...
 {{--menu mobil--}}
  <div class="sm:hidden" id="mobile-menu" x-show=open x-on:click.away="open=false">
    <div class="px-2 pt-2 pb-3 space-y-1">
     ...
     @foreach ($categorias as $categoria)
         <a href="{{route('posts.categoria', $categoria)}}" class="text-gray-300 hover:bg-gray-700 hover:text-white block px-3 py-2 rounded-md text-base font-medium">{{$categoria->name}}</a>
     @endforeach
